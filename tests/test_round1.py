...
class RoundOne(unittest.TestCase):
    ...
    def test_clan_and_registry_are_independent(self):
        ...
        self.assertEqual(canonical_name(p),'[完顏] 崇厚（地山）');self.assertIsNone(p['name']['surname']);self.assertTrue(p['name']['registration']['attestations'])
    def test_county_without_invented_registration(self):
        p=json.loads((ROOT/'data/people/person-zeng-jize.json').read_text());self.assertEqual(canonical_name(p),'[湘鄉] 曾紀澤（劼剛）')
