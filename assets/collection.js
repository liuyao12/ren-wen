...
try {
  ...
  function render(){
    ...
    $('library-results').innerHTML=matches.map(e=>`<article class="collection-entry"><h2><a href="${h(readerURL(e.source))}">${h(e.qualifier?'['+e.qualifier+'] ':'')}${h(e.name||e.title)}${h(e.alias?'（'+e.alias+'）':'')}</a></h2><p>${h(e.title)}</p><nav><a href="${h(readerURL(e.source))}">${english?'Read complete entry':'閱讀全文'}</a>${e.person?`<a href="${h(profileURL(e.person))}">${english?'Person profile':'人物資料'}</a>`:''}</nav>${e.qsg?`<small>${english?'QSG account or opening candidate linked in profile':'人物頁附清史稿相關記錄或開頭配對候選'}</small>`:''}</article>`).join('');
  }
