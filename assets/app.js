import {installSourceNavigation,resolveSource} from './source-navigation.js';
...
let loadSequence=0,sourceNavigation=null;
async function loadSource(id) {
  const seq=++loadSequence; const source=resolveSource(state.catalog,id);
  if(!source)throw Error('Unknown source.');
  if(source.kind==='cross-reference'){location.replace('library.html?alias='+encodeURIComponent(source.title.split(' · ')[0]));return;}
  id=source.id;sourceNavigation?.select(id);
  ...
  const reviewInfo=document.createElement('a');reviewInfo.href='review.html?source='+encodeURIComponent(source.id);reviewInfo.className='editorial-review-link';reviewInfo.textContent=getLocale()==='en'?'Identification review for this source':'本篇標註覆核';reader.querySelector('.source-caption').append(reviewInfo);
  const root=safeFragment(text);let last=0;
  for(const n of root.querySelectorAll('span[id]:not([data-entity])')){n.dataset.withheldIdentification='';for(const a of n.querySelectorAll('a'))a.replaceWith(...a.childNodes);}
...
window.addEventListener('renwen:navigate',e=>{const {witness,passage}=e.detail||{};if(!state.catalog?.sources.some(s=>s.id===witness))return;const url=new URL(location.href);url.search='';if(passage)url.searchParams.set('passage',passage);url.hash=witness;history.replaceState(null,'',url);loadSource(witness).catch(e=>toast(e.message));});
...
(async()=>{try{const r=await fetchData('data/catalog.json');if(!r.ok)throw Error(`Catalog request failed (${r.status}).`);await loadNameHistory();state.catalog=await r.json();sourceNavigation=installSourceNavigation(state.catalog,$('source-select'),id=>{const url=new URL(location.href);url.search='';url.hash=id;history.replaceState(null,'',url);loadSource(id).catch(e=>toast(e.message));});try{const stored=JSON.parse(localStorage.getItem(STORE)||'[]');state.queue=stored.filter(p=>{try{validateProposal(p,state.catalog);return true;}catch{return false;}});}catch{state.queue=[];}saveQueue();for(let lon=0;lon<=160;lon+=20)svg('line',{x1:lon,x2:lon,y1:0,y2:80,class:'gridline'},$('graticule'));for(let lat=15;lat<=75;lat+=15)svg('line',{x1:0,x2:170,y1:lat,y2:lat,class:'gridline'},$('graticule'));let id;try{id=decodeURIComponent(location.hash.slice(1));}catch{id='';}await loadSource(state.catalog.sources.some(s=>s.id===id)?id:state.catalog.defaultWitness);}catch(error){$('reader').innerHTML=`<h1>${ui("Unable to load Ren-Wen")}</h1><p class="error">${h(error.message)}</p><p>Serve this directory over HTTP, for example with <code>python -m http.server 8000</code>. Opening index.html as a local file cannot load the corpus.</p>`;$('reading-context').textContent='Loading failed';}})();
