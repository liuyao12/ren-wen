...
window.addEventListener('renwen:languagechange',()=>{
  ...
  const ctx=contextFor(state.catalog,state.source.id,state.passage);
  $('reading-context').textContent=`${state.source.shortTitle} · ${state.passage} · ${ctx.events.length ? [...new Set(ctx.events.map(e=>yearLabel(e.time)))].join(', ') : t('no single event date')}`;
});
