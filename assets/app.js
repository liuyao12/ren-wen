...
const link = (url,label) => safeURL(url) ? `<a href="${h(safeURL(url))}" target="_blank" rel="noopener">${h(label)} ↗</a>` : h(label);
function toast(text) { $('status').textContent=text; clearTimeout(state.timer); state.timer=setTimeout(()=>$('status').textContent='',6500); }
function modal(title,html) { $('dialog-body').innerHTML=`<h2>${h(title)}</h2>${html}`; if (!$('dialog').open) $('dialog').showModal(); }
...
function setYear(value, note) { state.year=Number(value);$('year').value=state.year;$('year-label').textContent=state.year;$('date-note').textContent=note||'Viewing year is not a date assigned to the entire passage.';renderTimeline(); }
...
function activate(id) {
  ...
  $('reading-context').textContent=`${state.source.shortTitle} · ${id} · ${ctx.events.length ? [...new Set(ctx.events.map(e=>yearLabel(e.time)))].join(', ') : 'no single event date'}`;
  if($('follow').checked && ctx.events.length) setYear(ctx.events[0].time.start, 'Following an event in this passage; not dating the whole paragraph.');
...
async function loadSource(id) {
  ...
  const reader=$('reader');reader.innerHTML=`<div class="kicker">${h(source.edition)}</div><h1>${h(source.title)}</h1><div class="source-caption">${h(source.extent)}<br>Annotations are proposals awaiting independent review. ${link(source.revisionUrl,'Read source')}</div>`;
  const root=safeFragment(text);let last=0;
  for(const p of [...root.children]) {
    const number=Number(p.id.split('-').at(-1));
    if(last&&number>last+1){const gap=document.createElement('div');gap.className='excerpt-gap';gap.textContent='· · · intervening text omitted · · ·';reader.append(gap);}
    last=number;p.lang=source.language;reader.append(p);
  ...
  reader.scrollTop=0;activate(reader.querySelector('p[id]')?.id);selectEntity(source.subject || state.catalog.defaultPerson);
...
function renderTimeline() {
  ...
  const allIds=new Set(state.catalog.mentions.filter(m=>m.witness===state.source.id).map(m=>m.entity));allIds.add(state.catalog.defaultPerson);
  ...
  people.forEach((p,i)=>{
    const active=ctx.entityIds.includes(p.id)||p.id===state.catalog.defaultPerson;const g=svg('g',{class:`person-row ${active?'active':'dim'}`,tabindex:0,role:'button','aria-label':`Inspect ${p.display}`},root);const y=i*60+40;
...
function renderEvents() {
  ...
  $('events').innerHTML=events.length?events.map(e=>`<button class="event-card" data-event="${h(e.id)}"><span class="date">${h(yearLabel(e.time))}</span><strong>${h(e.label)}</strong><small>${h(e.time.original)} · ${h(e.kind)} · proposed</small></button>`).join(''):'<p class="empty">No dated event has been extracted from this passage. No date is inferred just to fill the timeline.</p>';
}
function renderMap() {
  ...
  const here=places.filter(p=>ctx.entityIds.includes(p.id));$('places').innerHTML=here.length?here.map(p=>`<button class="place-card" data-place="${h(p.id)}"><strong>${h(p.display)} <span lang="zh-Hant">${h(p.label)}</span></strong><small>${p.point?'Modern reference point; historical jurisdiction not imported.':'Historical location / jurisdiction unresolved; no point invented.'}</small></button>`).join(''):'<p class="empty">No identified places in this passage.</p>';
}
function inspect(id,mentionId=null) {
  ...
  holder.innerHTML=`<div class="small-label">${h(e.type)} · ${mention?'occurrence-level annotation':'entity record'}</div><h2>${h(e.label)}</h2><p>${h(e.display)}</p>${e.note?`<p class="notice">${h(e.note)}</p>`:''}${e.life?`<p>Life dates: ${e.life.birth}–${e.life.death}<br>${link(e.life.source,'Evidence for the life bar')}</p>`:''}${e.point?`<p>${h(e.point.note)}<br>${link(e.point.source,'Coordinate reference')}</p>`:''}<p><code>${h(e.id)}</code></p>`;
  const sources=state.catalog.sources.filter(s=>(e.sources||[]).includes(s.id));
  if(sources.length||(e.externalSources||[]).length){holder.innerHTML+='<h3>Other accounts of this person</h3>';for(const s of sources)holder.innerHTML+=`<p><button data-source="${h(s.id)}">${h(s.shortTitle)}</button></p>`;for(const s of e.externalSources||[])holder.innerHTML+=`<p>${link(s.url,s.label)}</p>`;}
  const ms=state.catalog.mentions.filter(m=>m.entity===id&&m.witness===state.source.id);if(ms.length)holder.innerHTML+=`<h3>Mentions in this source</h3><div class="actions">${[...new Set(ms.map(m=>m.passage))].map(p=>`<button data-passage="${h(p)}">${h(p)}</button>`).join('')}</div>`;
  if(mention){holder.innerHTML+=`<h3>Review this occurrence</h3><blockquote>${h(mention.quote)}</blockquote><p>Status: ${h(mention.status)} · ${h(mention.origin)}<br><code>${h(mention.id)}</code></p><label for="candidate">Proposed identification<select id="candidate">${state.catalog.entities.filter(x=>x.type===e.type).map(x=>`<option value="${h(x.id)}" ${x.id===id?'selected':''}>${h(x.label)} · ${h(x.display)}</option>`).join('')}</select></label><label for="reason">Reason and supporting evidence<textarea id="reason" placeholder="Explain why this occurrence refers to a different entity…"></textarea></label><div class="actions"><button class="primary" id="propose">Add to review queue</button></div><p class="notice">This creates a local proposal. It does not alter the edition, publish to GitHub, or edit ctext.</p>`;
...
function showSourceInfo(){const s=state.source;modal('Source & edition',`<p><strong>${h(s.title)}</strong></p><p>${h(s.edition)}</p><p>${h(s.extent)}</p><h3>Import</h3><p>${h(s.importMethod)}</p><p>${link(s.revisionUrl,'Wikisource revision '+s.revision)} · retrieved ${h(s.retrieved)}</p><h3>Punctuation</h3><p>${h(s.punctuation.attribution)}</p><h3>Rights & attribution</h3><p>${h(s.rights.attribution)}<br>${h(s.rights.text)}<br>${h(s.rights.transcription)}</p><h3>Working-copy checksum</h3><p><code>${h(s.sha256)}</code></p><p class="notice">ECCP and 清史稿 are different accounts, not editions or translations of one another. The pilot has no aligned historical variants yet.</p>`);}
function showQueue(){modal('Review queue',`<p>Local proposals only. Export these for an agent or a reviewed repository change. Nothing is sent upstream.</p><div class="actions"><button id="export-queue" ${state.queue.length?'':'disabled'}>Export JSON</button><button id="import-queue">Import proposal JSON</button></div>${state.queue.length?state.queue.map(p=>`<div class="queue-item"><strong>${h(p.mention)}</strong><p>${h(entity(p.before)?.label)} → ${h(entity(p.after)?.label)}</p><p>${h(p.reason)}</p><button data-remove="${h(p.id)}">Remove proposal</button></div>`).join(''):'<p class="empty">Select a marked name, inspect its identification, and propose a correction.</p>'}`);$('export-queue').onclick=()=>download('ren-wen-proposals.json',JSON.stringify(state.queue,null,2)+'\n');$('import-queue').onclick=()=>$('proposal-file').click();}
...
$('events').addEventListener('click',e=>{const id=e.target.closest('[data-event]')?.dataset.event;const ev=state.catalog.events.find(x=>x.id===id);if(ev){setYear(ev.time.start,`${ev.time.original}. ${ev.note}`);modal('Event evidence',`<p><strong>${h(ev.label)}</strong></p><p>Source date: ${h(ev.time.original)}<br>Display index: ${h(yearLabel(ev.time))} (${h(ev.time.precision)} precision)</p><p>${h(ev.note)}</p><p>Status: ${h(ev.status)} · ${h(ev.kind)}</p><p>Evidence: ${h(ev.passage)} · ${link(state.source.revisionUrl,state.source.shortTitle)}</p><p class="notice">The display year is not a new assertion about every person or place in this paragraph.</p>`);}});
...
(async()=>{try{const r=await fetch('data/catalog.json');if(!r.ok)throw Error(`Catalog request failed (${r.status}).`);state.catalog=await r.json();$('source-select').innerHTML=state.catalog.sources.map(s=>`<option value="${h(s.id)}">${h(s.shortTitle)}</option>`).join('');try{const stored=JSON.parse(localStorage.getItem(STORE)||'[]');state.queue=stored.filter(p=>{try{validateProposal(p,state.catalog);return true;}catch{return false;}});}catch{state.queue=[];}saveQueue();for(let lon=0;lon<=160;lon+=20)svg('line',{x1:lon,x2:lon,y1:0,y2:80,class:'gridline'},$('graticule'));for(let lat=15;lat<=75;lat+=15)svg('line',{x1:0,x2:170,y1:lat,y2:lat,class:'gridline'},$('graticule'));let id;try{id=decodeURIComponent(location.hash.slice(1));}catch{id='';}await loadSource(state.catalog.sources.some(s=>s.id===id)?id:state.catalog.defaultWitness);}catch(error){$('reader').innerHTML=`<h1>Unable to load Ren-Wen</h1><p class="error">${h(error.message)}</p><p>Serve this directory over HTTP, for example with <code>python -m http.server 8000</code>. Opening index.html as a local file cannot load the corpus.</p>`;$('reading-context').textContent='Loading failed';}})();
